        ...
        )
{
    ...
    if (is_producer)
    {
        ...
        for (int load_stage = 0; load_stage<n_stages; load_stage++)
        {
            ...
            {
                int i = r*PRODUCER_THREADS*TO + (threadIdx.x)*TO;
        ...
        )
{
    ...
    gpuErrchk(cudaFuncSetAttribute(
        fused_moe_w8a8_wgmma_kernel<BM, BK, BN, WN, STAGES, PRODUCER_THREADS>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, sMemSize));

    fused_moe_w8a8_wgmma_kernel<BM, BK, BN, WN, STAGES, PRODUCER_THREADS><<<dimGrid, dimBlock, sMemSize>>>(
            x,
            x_scale,
            w,
            w_scale,
