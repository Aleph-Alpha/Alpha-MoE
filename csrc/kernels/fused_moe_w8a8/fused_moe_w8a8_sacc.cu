...
struct smem
{
    ...
    uint64_t bar[STAGES];
    float2 acc[TN][2][WN*WM*32];
};

template <int BM, int BK, int BN, int PF, int WM, int WN, int STAGES, int TN>
__global__ __launch_bounds__(128) void fused_moe_w8a8_sacc_kernel(
        ...
        )
{
    ...
    // __shared__ float2 s_acc[TB][2][WN*WM*32];
    // N tiles in memory
    constexpr int STG = block_shape[0]/32;

    extern __shared__ smem<STAGES, WS, XS, WM, WN, TN> s;
    if (threadIdx.x == 0)
    {
        for (int i = 0; i < STAGES; i++)
        {
            init_barrier(&s.bar[i], blockDim.x, 0);
        }
    }
    __syncthreads();
    ...
    auto async_load = [&]()
    {
        ...
        cp_async_mbarrier_arrive(s.bar + smem_stage);
        load_stage++;
    };
    ...
    for (int block=0; block < n_stages; block += 1)
    {
        ...
        wait(s.bar + (compute_stage%STAGES), p);
        ...
        for(int w_off = 0; w_off<TN; w_off++)
        {
            ...
            if (token_src[0] < M)
            {
                float2 f_a = block == 0 ? make_float2(0.0f, 0.0f) : s.acc[w_off][0][threadIdx.x];
                f_a.x += scale_x[0] * scale_w * ta[0];
                f_a.y += scale_x[0] * scale_w * ta[1];
                s.acc[w_off][0][threadIdx.x] = f_a;
                // f_acc[w_off][0] += scale_x[0] * scale_w * ta[0];
                // f_acc[w_off][1] += scale_x[0] * scale_w * ta[1];
            }
            if (token_src[1] < M)
            {
                float2 f_a = block == 0 ? make_float2(0.0f, 0.0f) : s.acc[w_off][1][threadIdx.x];
                f_a.x += scale_x[1] * scale_w * ta[2];
                f_a.y += scale_x[1] * scale_w * ta[3];
                s.acc[w_off][1][threadIdx.x] = f_a;
    ...
    for(int i = 0; i<TN; i++)
    {
        if (token_src[0] < M)
        {
            float2 fa = s.acc[i][0][threadIdx.x];
            *reinterpret_cast<__nv_bfloat162*>(out + token_dest[0]*N + warpN * BN + i*8 + (lane_id%4)*2) = __nv_bfloat162(fa.x, fa.y);
        }
        if (token_src[1] < M)
        {
            float2 fa = s.acc[i][1][threadIdx.x];
            *reinterpret_cast<__nv_bfloat162*>(out + token_dest[1]*N + warpN * BN + i*8 + (lane_id%4)*2) = __nv_bfloat162(fa.x, fa.y);;
        }
    }
        ...
        )
{
    ...
    gpuErrchk(cudaFuncSetAttribute(
        fused_moe_w8a8_sacc_kernel<BM, BK, BN, PF, WM, WN, STAGES, TN>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, sMemSize));


    fused_moe_w8a8_sacc_kernel<BM, BK, BN, PF, WM, WN, STAGES, TN><<<dimGrid, dimBlock>>>(
            x,
            x_scale,
            w,
            w_scale,
